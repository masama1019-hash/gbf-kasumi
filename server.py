...
# 出典: ゲームの団員一覧(2026-09-19時点の30名。とくこ→Zako、皇杞枢は陽見恵凪に改名)
MEMBERS = [
    ("Laphis", 3052899), ("明蓮", 2090466), ("ibls", 2722700),
    ("ノイン", 8884374), ("ジーク", 4148920), ("yamato", 1240007),
    ("クラシック１", 3767386), ("えりゅ", 10408332), ("Ryan", 10219632), ("きぃ", 929001), ("はりま", 204160),
    ("幼女新撰組", 9727079), ("nao", 12607435), ("藤城真香", 27769761),
    ("さくらえび", 7628906), ("カノン", 16101483), ("だよだよめん", 6710599),
    ("すなっ", 3557547), ("ジータ", 34524668), ("D.N", 2569839),
    ("ルルフォン", 1490546), ("ねろそーす", 3740709), ("紅蘭", 16483149),
    ("陽見恵凪", 22967234), ("しびー", 23600641), ("風色幻想", 25557296),
    ("レン", 12275867), ("じゅりこ", 38776098), ("さとう", 31919490), ("Zako", 14126334),
]
...
def api_live(q):
    ...
    # ---- 相手方針判定(時速ベース) & 行動パターン ----
    # 本日勝率予想は 2026-09-18 にユーザー指定で廃止。残り時間・最高時速・行動パターン・
    # 過去ペースで組む版まで作ったが表示しない判断。復活させるなら dc09bf4 を参照
    forecast = None
...
# (81回以前)の「直近6回の着地」参考用。出典: グランブルーファンタジー.gamewith.jp
# /article/show/91154 (第82・83回はgbfdata実値と一致で検証済み)
# 開催回セレクタは第63回まで遡れ、その6回前=第57回まで要るので第48回まで収録。
# b100kは10万位【勲章】。第76回以前は勲章の順位帯自体が違う(76〜68回=9万位、
# 67〜57回=8万位、56〜50回=7万位、49回以前=5万位)ため2000位のみ収録する。
GW_BORDER_FINAL = {
    83: {"b2000": 502.3, "b100k": 64.6}, 82: {"b2000": 456.3, "b100k": 70.5}, 81: {"b2000": 361.2, "b100k": 50.3},
    80: {"b2000": 234.7, "b100k": 33.1}, 79: {"b2000": 241.4, "b100k": 31.3}, 78: {"b2000": 179.4, "b100k": 23.8},
    77: {"b2000": 219.4, "b100k": 34.7}, 76: {"b2000": 193.0}, 75: {"b2000": 148.6},
    74: {"b2000": 161.3}, 73: {"b2000": 155.1}, 72: {"b2000": 139.3},
    71: {"b2000": 93.3}, 70: {"b2000": 121.9}, 69: {"b2000": 93.1},
    68: {"b2000": 76.7}, 67: {"b2000": 82.3}, 66: {"b2000": 114.4},
    65: {"b2000": 60.1}, 64: {"b2000": 52.7}, 63: {"b2000": 48.1},
    62: {"b2000": 50.8}, 61: {"b2000": 55.6}, 60: {"b2000": 50.1},
    59: {"b2000": 35.1}, 58: {"b2000": 29.4}, 57: {"b2000": 33.5},
    56: {"b2000": 28.5}, 55: {"b2000": 21.8}, 54: {"b2000": 25.7},
    53: {"b2000": 23.6}, 52: {"b2000": 17.2}, 51: {"b2000": 19.8},
    50: {"b2000": 16.2}, 49: {"b2000": 16.3}, 48: {"b2000": 13.4},
}
